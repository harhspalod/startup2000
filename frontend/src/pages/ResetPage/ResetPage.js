...
const ResetPage = () => {
      ...
      if (id && token) {
        ...
        const Verify = async () => {
          const { data } = await axios.post(
            `https://startopia-backend.onrender.com/api/v1/auth/user/${id}/${token}`
          );
          if (data.success) {
  ...
  const onSubmit = async () => {
    if (formvalue.active === 0) {
      if (formvalue.email) {
        const { data } = await axios.post(
          "https://startopia-backend.onrender.com/api/v1/auth/user",
    ...
    if (formvalue.active === 2) {
      if (
        ...
        const { data } = await axios.post(
          `https://startopia-backend.onrender.com/api/v1/auth/resetpassword`,
